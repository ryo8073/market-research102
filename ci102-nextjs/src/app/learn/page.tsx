        ...
        <main className="flex-1 min-w-0 px-4 py-6 md:px-6 space-y-6">
          ...
          <Section id="ch10-trade-area" title="第10章: カスタム経済圏・商圏分析 — 物件単位の判断へ">
            ...
            <InterpTable
              ...
              rows={[
                ["⑫ カスタム経済圏", "近隣自治体を合算した経済圏評価", "市区町村をチェックボックスで複数選択", "湘南エリア（藤沢+鎌倉+茅ヶ崎+平塚）の合算評価"],
                ["⑬ 商圏分析", "物件位置から半径X km圏内の経済集計", "住所 or 緯度経度 + 半径選択", "特定物件の徒歩商圏（1km）/ 広域商圏（10km）"],
              ]}
            />

            <h3 className="font-semibold text-lg mt-6">カスタム経済圏（タブ⑫）の使い方</h3>
            <p>
              都市圏分析（タブ⑪）が「東京圏=1都3県」のような固定エリアなのに対し、
              カスタム経済圏は<strong>分析者が自由に範囲を定義</strong>できます。
            </p>
            <ol className="list-decimal list-inside space-y-1">
            ...
            <h3 className="font-semibold text-lg mt-6">商圏分析 vs 都市圏分析（タブ⑪） vs カスタム経済圏（タブ⑫）</h3>
            <InterpTable
              headers={["タブ", "範囲定義", "粒度", "適した分析"]}
              rows={[
                ["⑪ 都市圏(MSA)", "事前定義の7圏（東京/大阪/名古屋等）", "都道府県合算", "マクロな経済圏比較・地方の中核都市の独立性検証"],
                ["⑫ カスタム経済圏", "市区町村を自由選択", "市区町村単位", "独自エリア定義（湘南・道南等）・近隣自治体合算"],
