    ...
    <div className="min-h-screen bg-[#F8F9FA] dark:bg-gray-950">
      ...
      <div className="max-w-7xl mx-auto flex">
        ...
        <main className="flex-1 min-w-0 px-4 py-6 md:px-6 space-y-6">
          ...
          <Section id="ch9-granularity" title="第9章: 業種分類粒度とCI102分析 — なぜ数値が大きく変わるのか">
            ...
            <InterpTable
              ...
              rows={[
                ...
                ["東京圏（首都圏 1都3県合算）", "11.41", "8.8%", "9", "960", "2.10"],
                ["名古屋圏（中京）", "11.51", "8.7%", "2", "1,242", "1.60"],
                ["仙台都市圏", "15.23", "6.6%", "9", "1,038", "1.63"],
                ["福岡都市圏", "21.40", "4.7%", "7", "1,082", "1.32"],
                ["広島都市圏", "22.98", "4.4%", "8", "1,123", "1.37"],
                ["大阪圏（京阪神 2府2県合算）", "28.98", "3.5%", "7", "1,093", "1.17"],
              ]}
            />
            <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 mt-2 text-sm">
              <p className="font-semibold mb-1">📌 重要: 都道府県単独で見ると EBM はもっと低い (=教科書範囲に近い)</p>
              <table className="w-full text-xs mt-2">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-1 pl-2">教科書(Orlando 4.94)との関係</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b"><td className="py-1">東京都<strong>単独</strong></td><td className="text-right font-bold text-emerald-700">5.79</td><td className="text-right">17.3%</td><td className="pl-2 text-emerald-700">✓ 範囲内</td></tr>
                  <tr className="border-b"><td className="py-1">東京圏1都3県<strong>合算</strong></td><td className="text-right">11.41</td><td className="text-right">8.8%</td><td className="pl-2 text-amber-700">⚠ 合算で上振れ</td></tr>
                  <tr className="border-b"><td className="py-1">大阪府<strong>単独</strong></td><td className="text-right text-amber-700">20.57</td><td className="text-right">4.9%</td><td className="pl-2 text-amber-700">既に範囲外</td></tr>
            ...
            <CaseStudy title="教科書 Orlando MSA との対比 — 特化パターンの違い">
              ...
              <p className="mt-3 mb-2">
                <strong>東京都単独</strong> EBM <strong>5.79</strong> / 基盤率 17.3% / 基盤産業 6業種 — <em>教科書範囲内</em>
              </p>
              <ul className="list-disc list-inside text-sm space-y-1">
                <li>教科書 Orlando MSA 4.94 / 基盤率 20.2% に近い「健全圏」</li>
                <li>金融・情報通信・専門サービス等で深く特化</li>
              </ul>

              <p className="mt-3 mb-2">
                <strong>東京圏 (1都3県合算)</strong> EBM <strong>11.41</strong> / 基盤率 8.8% / 基盤産業 9業種 / 最大LQ 2.10
              </p>
              <ul className="list-disc list-inside text-sm space-y-1">
                <li>9業種で<strong>広く中程度に特化</strong> (基盤業種数は単独より増える)</li>
                <li>しかし周辺県を加えると地域シェアが全国平均に近づき、LQ&gt;1業種の優位性が薄まる</li>
                <li>結果として基盤雇用比率が 8.8% に下がり、EBM が単独の 5.79 から 11.41 に「上がって」見える</li>
              </ul>
              <p className="mt-3">
                <strong>どちらも『多角化された健全な大都市圏』</strong>だが、特化のパターンが異なります。
                東京都単独は Orlando と同じ「狭く深い」特化、東京圏合算は「広く中程度」の特化。
                日本固有の東京一極集中構造により、日本の他都市圏は『LQが頭打ち』になり、特化が浅い。
              </p>
            </CaseStudy>

            <h4 className="font-semibold mt-6">なぜ大阪圏のEBMが高い (大阪府単独20.57 → 大阪圏28.98) のか</h4>
            <p>
              大阪府<strong>単独</strong>でも既に EBM 20.57 / 基盤率 4.9% で教科書範囲外。
              <strong>大阪圏 (2府2県合算)</strong> ではさらに上がって EBM 28.98 / 基盤率 3.5%。
              これは大阪が「全国第2位の経済圏」ではあるものの、<strong>東京一極集中の影響で
              主要産業が全国シェアと同等になっている</strong>ことを反映します:
            </p>
            <ul className="list-disc list-inside space-y-1">
              <li>金融商品取引業の本社機能は東京に集中 → 大阪は LQ 1.1 程度</li>
              <li>情報サービス業も東京集中 → 大阪は LQ 0.9 程度</li>
              <li>製造業は近郊（兵庫・京都）に分散しているが、大分類で見ると LQ ≈ 1.0</li>
              <li>結果として基盤産業 9業種 (単独) → 7業種 (合算)、最大LQ 1.17 と『弱い特化が点在』</li>
            </ul>
            <p>
              これは『大阪が衰退している』ではなく、<strong>『大阪の主要産業が全国シェアと同等』
              という意味</strong>。LQの計算原理上、全国シェアと同等の業種は基盤雇用に算入されない
              ため、見かけ上 EBM が高くなります。圏域合算でさらに「全国シェアに近づく」ため、EBM が単独より上がります。
            </p>
            ...
            <ClientTip>
              ...
              <p>
                ...
                <strong>東京都単独 EBM 5.79</strong> は教科書 Orlando MSA 4.94 と近い「健全圏」、
                <strong>東京圏(1都3県)合算 EBM 11.41</strong> は周辺県を加えた数学的副産物。
                <strong>大阪府単独 EBM 20.57</strong> も既に高く、これは東京一極集中で大阪府の主要産業が全国シェアと同等になっているサイン。
          ...
          {/* ======================== 第10章: カスタム経済圏・商圏分析 ======================== */}
          <Section id="ch10-trade-area" title="第10章: カスタム経済圏・商圏分析 — 物件単位の判断へ">
            <h3 className="font-semibold text-lg">なぜ「経済圏」「商圏」を独自に定義するのか</h3>
            <p>
              CI102分析は地域経済の構造を測定しますが、不動産投資の<strong>現場の意思決定単位</strong>は通常、
              市区町村や都道府県ではなく『物件から見える範囲』『お客様が通う範囲』です。
              そのため本ツールでは2種類のカスタム範囲分析を提供します。
            </p>
            <InterpTable
              headers={["機能", "用途", "範囲指定方法", "想定シナリオ"]}
              rows={[
                ["🌐 経済圏分析", "近隣自治体を合算した経済圏評価", "市区町村をチェックボックスで複数選択", "湘南エリア（藤沢+鎌倉+茅ヶ崎+平塚）の合算評価"],
                ["📍 商圏分析", "物件位置から半径X km圏内の経済集計", "住所 or 緯度経度 + 半径選択", "特定物件の徒歩商圏（1km）/ 広域商圏（10km）"],
              ]}
            />

            <h3 className="font-semibold text-lg mt-6">経済圏分析（🌐タブ）の使い方</h3>
            <p>
              都市圏分析（🏙️タブ）が「東京圏=1都3県」のような固定エリアなのに対し、
              経済圏分析は<strong>分析者が自由に範囲を定義</strong>できます。
              投資判断ハブ（🎯タブ）にも経済圏モードが統合されており、プリセット選択で即座に合算分析が可能です。
            </p>
            <ol className="list-decimal list-inside space-y-1">
              <li>左側の県セレクターで対象県を選ぶ（複数県をまたいだ選択も可能）</li>
              <li>市区町村のチェックボックスを選択（「県内すべて」「全クリア」ボタンも）</li>
              <li>右側に即座に合算結果が表示（EBM、基盤雇用比率、多角化指標、基盤産業TOP10）</li>
            </ol>

            <CaseStudy title="実例: 湘南エリア vs 神奈川県全体">
              <p>
                <strong>神奈川県単体</strong>: EBM 18.21 / 基盤率 5.5% / 通勤流出ベッドタウン判定
              </p>
              <p className="mt-2">
                <strong>湘南エリア（藤沢+鎌倉+茅ヶ崎+平塚）合算</strong>: 4市の経済を統合すると、
                神奈川県全体（横浜・川崎の通勤流出の影響）よりも『独立した観光・住宅地経済圏』としての
                性格が見えやすくなります。
              </p>
              <p className="mt-2 text-sm">
                ※ 都道府県全体やMSA（東京圏）では捉えきれない『独自エリアブランディング』の数値根拠を作るのに有効。
              </p>
            </CaseStudy>

            <h3 className="font-semibold text-lg mt-6">商圏分析（📍タブ）の使い方</h3>
            <p>
              物件の住所から指定半径内の市区町村を自動判定して経済集計します。
            </p>
            <ol className="list-decimal list-inside space-y-1">
              <li>住所を入力（例「東京都千代田区丸の内1-9-1」）→「検索」ボタンで緯度経度取得
                <ul className="list-disc list-inside ml-4 text-sm text-slate-600 mt-1">
                  <li>国土地理院 Web API（無料・無認証）を利用</li>
                  <li>緯度経度を直接入力することも可能（折りたたみセクション）</li>
                </ul>
              </li>
              <li>半径を選択: 1km（徒歩）/ 2km（自転車）/ 5km（近隣）/ 10km（広域）/ 30km（都市圏）</li>
              <li>地図に中心ピン + 半径円 + 圏内市区町村セントロイドが表示</li>
              <li>右側に経済集計（EBM・HHI・基盤産業TOP10・解釈ガイド）が即時表示</li>
            </ol>

            <h3 className="font-semibold text-lg mt-6">半径の選び方の指針</h3>
            <InterpTable
              headers={["半径", "目安", "用途", "想定物件タイプ"]}
              rows={[
                ["1km", "徒歩 約15分", "近隣住民の購買力分析", "コンビニ・ドラッグストア・カフェ"],
                ["2km", "徒歩+自転車 20-30分", "日常生活商圏", "スーパー・小規模商業施設・賃貸住宅"],
                ["5km", "車 10-15分", "近隣商圏（GMS・量販店）", "ショッピングセンター・郊外型店舗"],
                ["10km", "車 20-30分", "広域商圏", "大型ショッピングモール・物流倉庫"],
                ["30km", "車 50-60分", "都市圏全体", "オフィスビル・地域中核病院"],
              ]}
            />
            ...
            <h3 className="font-semibold text-lg mt-6">クライアント説明テンプレート</h3>
            <ClientTip>
              <p className="mb-2"><strong>物件単位の評価が必要なお客様への説明</strong>:</p>
              <p>
                「お問い合わせの物件位置から半径{"{"}X{"}"}kmの商圏を分析しました。
                圏内には{"{"}N{"}"}市区町村が含まれ、総雇用{"{"}emp{"}"}万人の経済基盤があります。
                <br />
            ...
            <h3 className="font-semibold text-lg mt-6">商圏分析 vs 都市圏分析 vs 経済圏分析</h3>
            <InterpTable
              headers={["タブ", "範囲定義", "粒度", "適した分析"]}
              rows={[
                ["🏙️ 都市圏(MSA)", "事前定義の7圏（東京/大阪/名古屋等）", "都道府県合算", "マクロな経済圏比較・地方の中核都市の独立性検証"],
                ["🌐 経済圏分析", "市区町村を自由選択（🎯投資判断ハブにもプリセット統合）", "市区町村単位（中分類95業種）", "独自エリア定義（湘南・道南等）・近隣自治体合算"],
                ["📍 商圏分析", "物件位置 + 半径", "市区町村セントロイド", "物件単位の投資判断・特定店舗の商圏推定"],
              ]}
            />
            <p>
              <strong>使い分けのコツ</strong>: 都市圏 → カスタム → 商圏 と、<strong>段階的にズームインして物件評価</strong>する流れが効果的です。
            </p>

            <div className="rounded-lg border-l-4 border-l-amber-400 bg-amber-50 dark:bg-amber-950/20 p-4 mt-4">
              <p className="font-semibold text-amber-700">重要な制限事項</p>
              <ul className="list-disc list-inside text-sm space-y-1 mt-2">
                <li>商圏分析はセントロイドで判定するため、大面積の市区町村（北海道郡部等）では境界の一部が半径外でも全体が圏内に含まれる</li>
                <li>haversine（直線距離）は地形・道路網・河川・湾岸を考慮しない。山岳地・島嶼部では走行時間ベースとの乖離が大きい</li>
                <li>国土地理院ジオコーディング API は完全一致ではないため、都道府県名から入力推奨</li>
                <li>カスタム経済圏は大分類17業種で計算（中分類版は実装未対応）</li>
