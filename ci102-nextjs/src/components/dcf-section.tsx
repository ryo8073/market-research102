    ...
    <div className="space-y-3">
      ...
      <div className="rounded border-2 border-purple-400 bg-white p-3">
        ...
        <div className="grid md:grid-cols-3 gap-2 text-xs">
          <div className="rounded bg-blue-50 p-2">
            <p className="text-xs text-slate-500">マクロ (地域経済)</p>
            <p className="text-lg font-bold">{macroScore.toFixed(0)} / 100</p>
            <p className="text-xs text-slate-600">最適: {macroPropertyType}</p>
          </div>
          <div className="rounded bg-purple-50 p-2">
            <p className="text-xs text-slate-500">ミクロ (物件収益)</p>
            <p className="text-lg font-bold">{microScore.toFixed(0)} / 100</p>
            <p className="text-xs text-slate-600">Cap+DSCR+IRRから算出</p>
          </div>
          <div className="rounded p-2" style={{ backgroundColor: integratedVerdict.bg }}>
            <p className="text-xs text-slate-500">統合判定</p>
            <p className="text-lg font-bold" style={{ color: integratedVerdict.color }}>
              {integratedVerdict.label}
            </p>
            <p className="text-xs text-slate-600">{integratedVerdict.note}</p>
          </div>
        </div>
      </div>
      ...
      {data.memo && (
        <details className="text-xs">
          ...
          <pre className="mt-1 p-2 bg-white border rounded whitespace-pre-wrap text-xs">{data.memo}</pre>
        </details>
      )}

      {data.disclaimer && (
        <p className="text-xs text-slate-500 italic">{data.disclaimer}</p>
      )}
    </div>
  );
}
